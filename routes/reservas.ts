...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        if (reserva.status !== StatusReserva.PENDENTE) {
            return res.status(403).json({ erro: "Não é possível cancelar uma reserva que já foi confirmada ou cancelada." });
        }
...
// Rota para o ADMIN aprovar ou cancelar uma reserva
